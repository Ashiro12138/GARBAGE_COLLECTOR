...
public void draw() {
  if (!gameStarted) {
    if (!zoomAnimationPlaying) {
      ...
      image(menuBg, -50, 0);
      if (menuOption == 0) {
        image(pointerOption, 90, -20, 800 * 0.8f, 600 * 0.8f);
      } else {
        image(stackOption, 90, -20, 800 * 0.8f, 600 * 0.8f);
      }
      ...
      if (now.getTime() - animationStart.getTime() > 500) { //milliseconds time out to play animation
        ...
        if (menuOption == 0) { //pointer game
          image(enterIP, 90, -20, 800 * 0.8f, 600 * 0.8f); 
          game = new PointerGame(this);
        } else if (menuOption == 1) {
          image(menuBg, -50, 0);
          image(serverWait, 90, -20, 800 * 0.8f, 600 * 0.8f);
    ...
    // Check Powerups 
    eye.update();
    eye.checkCollision(stkBoi.x,stkBoi.y, 34, 34);
    boots.update();
    boots.checkCollision(stkBoi.x,stkBoi.y, 34, 34);
    bomb.update();
    bomb.checkCollision(stkBoi.x,stkBoi.y, 34, 34);
    heart.update();
    heart.checkCollision(stkBoi.x,stkBoi.y, 34, 34);
     ...
     if (ptrBoi.x != lastX || ptrBoi.y != lastY) {
      ...
      for (String new_data: datas){
        ...
        if (cmd.equals("pos")) {
           ...
           stkBoi.health = PApplet.parseInt(data[1]);
           stkBoi.setSkin();
